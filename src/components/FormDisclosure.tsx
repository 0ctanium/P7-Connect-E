...
export const FormDisclosure: FC<{ label: string }> = ({ label, children }) => {
  return (
    <Disclosure>
      {({ open }) => (
        <>
          <Disclosure.Button className="group flex py-2 text-sm font-medium text-left text-fiord-600 hover:text-fiord-500">
            <ChevronUpIcon
              className={`${
                open ? 'transform rotate-180' : ''
              } w-5 h-5 text-fiord-600 group-hover:text-fiord-500`}
            />
            <span>{label}</span>
          </Disclosure.Button>
