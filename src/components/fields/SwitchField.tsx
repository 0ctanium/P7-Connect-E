      ...
      label || desc ? (
        <span
          ...
          {desc && (
            <Switch.Description
              as="span"
              className={clsx('text-sm text-gray-500', classes?.desc)}>
              {desc}
            </Switch.Description>
          )}
        </span>
          ...
          <span
            className={clsx(
              checked
                ? 'opacity-100 ease-in duration-200'
                : 'opacity-0 ease-out duration-100',
              'absolute inset-0 h-full w-full flex items-center justify-center transition-opacity text-fiord-600',
              classes?.iconOn
            )}
            aria-hidden="true">
            {/* @ts-ignore */}
    ...
    return (
      <Switch.Group
        ...
        <Switch
          ...
          className={clsx(
            !short && (checked ? 'bg-fiord-600' : 'bg-gray-200'),
            'relative inline-flex flex-shrink-0 rounded-full cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-fiord-500',
            short
          ...
          {short ? (
            <Fragment>
              ...
              <span
                aria-hidden="true"
                className={clsx(
                  checked ? 'bg-fiord-600' : 'bg-gray-200',
